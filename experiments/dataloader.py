...
def load_dataset(dataset_dir, batch_size, position_file, val_batch_size=None, include_context=False, knn=True,  **kwargs):
    ...
    if knn:
        with open(position_file, 'rb') as f:
            lonlat = joblib.load(f)['lonlat']
    else:
        with open(position_file, 'rb') as f:
            pos = joblib.load(f)['lonlat']
            idx = pos['idx']
            dis = pos['dis']
...
class Dataset(Dataset):
    ...
    def __init__(self, dataset_dir, mode, batch_size, scaler=None, include_context=False,
        ...
        if mode == 'trn':
            eet = np.load(eet_dir)
            self.e, self.u = np.linalg.eig(eet)
