...
from model.structure.network.base_model import SAMSGL
...
class Supervisor:
    def __init__(self, **kwargs):
        ...
        if self.knn:
            ...
            self.sparse_idx = torch.from_numpy(self._data['kernel_info'][0]).long().to(self._device)  # 2,100*k
            self.location_info = 0
            self.geodesic = torch.from_numpy(self._data['kernel_info'][1]).float().to(self._device)
            self.angle_ratio = 0
    ...
    def _test_final_n_epoch(self, n=5, steps=[12, 20, 30]):
        model_path = Path(self._log_dir)/'saved_model'
    ...
    def _get_time_prediction(self):
        ...
        with torch.no_grad():
            ...
            # delays = torch.cat(delays, 0).cpu().numpy()
            # weights = torch.cat(weights, 0).cpu().numpy()
